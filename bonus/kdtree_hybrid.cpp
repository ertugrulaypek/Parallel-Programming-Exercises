...
// change distance_squared to use immintrin functions with aligned load
float Point::distance_squared(Point &a, Point &b)
{
    int ub = (a.dimension) - ((a.dimension) % 8);

    __m256 va, vb, tmp, tmp2, partial_sum;
    partial_sum = _mm256_set1_ps(0);
...
// customize problem generation to use aligned load
Point** custom_generate_problem(int seed, int dim, int num_points, int start, int end, int num_queries, Point queryPoints[]){
    ...
    int coordSize = dim * sizeof(float);

    for(int n = 0; n < num_points; ++n){

        // set MPI process' local tree points
        if (n >= start && n < end) {
            float* coords = (float*)aligned_alloc(32, coordSize);
            for(int d = 0; d < dim; ++d) {
                *(coords + d) = distribution(random);
            }
            points[n - start] = new Point(dim, n + 1, coords);
        }

        // set query points
        else if (n >= num_points - num_queries) {
            float* coords = (float*)aligned_alloc(32, coordSize);
            for(int d = 0; d < dim; ++d) {
                *(coords + d) = distribution(random);
            }
            queryPoints[n - num_points + num_queries] = Point(dim, n + 1, coords);
        }

        // dummy branch to get random distribution into the same state in all MPI processes
        else {
            for(int d = 0; d < dim; ++d) {
                distribution(random);
            }
...
int main(int argc, char **argv){
    ...
    else // WORKER ROUTINE
    {
        ...
        Point queryPoints[num_queries];
        Point** points = custom_generate_problem(seed, dim, num_points + num_queries, start, end, num_queries, queryPoints);
